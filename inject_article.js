...
const filePath = path.join(__dirname, 'lib', 'artikel.ts');
let content = fs.readFileSync(filePath, 'utf8');

// The new article object to add
const newArticle = `
export const artikelVersus = {
  ...
  headerCta: "https://wa.me/6287778999141?text=Halo%20kak,%20saya%20mau%20pasang%20XL%20SATU",
  bodyHtml: \`
...
// Insert the new article right before the final export statement in artikel.ts
const exportIndex = content.indexOf('export const SEMUA_ARTIKEL');
if (exportIndex !== -1) {
  content = content.slice(0, exportIndex) + newArticle + content.slice(exportIndex);
  content = content.replace('export const SEMUA_ARTIKEL = [', 'export const SEMUA_ARTIKEL = [\n  artikelVersus,');
  fs.writeFileSync(filePath, content);
  console.log('Successfully injected new article.');
} else {
  console.error('Could not find SEMUA_ARTIKEL export');
}
